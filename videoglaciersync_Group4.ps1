...
$logName = "video_hi-res_Group4_$logDate.log"
$deltaArray = $vidArray[15..19]

Start-Log -LogPath "\\DATA2\AVD_Dept\MIKE\Logs" -LogName "$logName" -ScriptVersion "1.0"


foreach ($collection in $deltaArray){
...
$notification = @{
            ...
            body = "video_hi-resGroup4 synced to glacier at $timeComplete"
        }
$pbkey = '[REDACTED]'
