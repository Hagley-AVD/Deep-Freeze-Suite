...
$logName = "video_hi-res_Group2_$logDate.log"
$betaArray = $vidArray[5..9]

Start-Log -LogPath "\\DATA2\AVD_Dept\MIKE\Logs" -LogName "$logName" -ScriptVersion "1.0"


foreach ($collection in $betaArray){
...
$notification = @{
            ...
            body = "video_hi-resGroup2 synced to glacier at $timeComplete"
        }
$pbkey = '[REDACTED]'
